...
Problem $$P$$ is split into sub-problems $$P_o, P_e$$, each having half the size of $$P$$. Recursively solve $$P_o, P_e$$ leads to a problem with $$O(n\log n)$$ complexity.
...
The bottom layer of the problem would receive inputs $$[x_1^4]$$.
...
> > 
> >
> > where $$i$$ is the [imaginary unit](https://en.wikipedia.org/wiki/Imaginary_unit). In other words, rotating a vector on complex plane around origin is equal to multiplication with certain unit vector. $$\omega_8^1=e^{i\pi/4}$$ rotates itself $$\pi/4$$ counter-clockwise.
